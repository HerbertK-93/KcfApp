...
class _OnceScreenState extends State<OnceScreen> {
  ...
  Future<void> _initiatePayment() async {
    ...
    try {
      ...
      final paymentResponse = await _flutterwaveService.initiatePayment(
        ...
        transactionType: 'Once',  // Specify that this is a monthly saving plan
      );
  ...
  Stream<QuerySnapshot> _getOnceSavingsStream(bool isLocal) {
    String currency = isLocal ? 'UGX' : 'USD';  // Local uses UGX, Intl uses USD
    return FirebaseFirestore.instance
        .collection('users')
        .doc(FirebaseAuth.instance.currentUser?.uid)
        .collection('transactions')
        .where('transaction_type', isEqualTo: 'Once')
  ...
  Widget _buildOnceSavingsList(bool isLocal) {
    return StreamBuilder<QuerySnapshot>(
      stream: _getOnceSavingsStream(isLocal),
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return const Text('No one time savings found.');
        }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildOnceSavingsList(true),  // Local savings list
          ],
        ),
      ),
    );
    ...
    return SingleChildScrollView(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Text(
              'One-time Savings (International)',
              style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
            ),
            const SizedBox(height: 10),
            TextField(
            ...
            const SizedBox(height: 20),
            _buildOnceSavingsList(false),  // International savings list
          ],
        ),
      ),
    );
